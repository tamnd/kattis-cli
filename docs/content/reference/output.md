...
describes what users get. Pick a format with `-o`, or let kattis choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
kattis <command> -o table   # aligned columns for reading
kattis <command> -o jsonl   # one JSON object per line, for piping
kattis <command> -o json    # a single JSON array
kattis <command> -o csv     # spreadsheet friendly
kattis <command> -o tsv     # tab-separated
kattis <command> -o url     # just the URL column
kattis <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
kattis <command> --fields id,title,url
```
...
```bash
kattis <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
kattis <command>            # a table, because this is a terminal
kattis <command> | wc -l    # JSONL, because this is a pipe
```
