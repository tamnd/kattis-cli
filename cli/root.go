// Package cli builds the kattis command tree on top of the kattis library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: kattis.DefaultConfig()}

	root := &cobra.Command{
		Use:   "kattis",
		Short: "Browse Kattis competitive programming problems",
		Long: `kattis browses the Kattis Online Judge at https://open.kattis.com/ — a
competitive programming platform with thousands of problems from beginner to
expert level.

No account or API key is required. Output can be formatted as a table, JSON,
JSONL, CSV, TSV, or URL list.

kattis is an independent tool and is not affiliated with Kattis or its authors.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = kattis.NewClient(a.cfg)
	return nil
}
